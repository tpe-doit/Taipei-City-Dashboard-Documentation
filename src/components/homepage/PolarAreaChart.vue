...
<template>
	...
	<div v-if="activeChart === 'PolarAreaChart'" class="polarareachart">
		...
		<svg class="polarareachart-chart" xmlns="http://www.w3.org/2000/svg">
			<g v-for="(sector, index) in sectors" :key="index">
				<path :class="{
					...
					sector: true,
				}" v-if="sector.show" :d="sectorD(index)" :fill="sector.fill" :stroke-width="sector.stroke_width"
			...
			<g v-for="(label, index) in labels" :key="index">
				<text :x="label.x" :y="label.y" text-anchor="middle" alignment-baseline="middle" fill="#888787"
					font-size="12">
					{{ label.name }}
				</text>
			</g>
		...
		<div class="polarareachart-legend" v-if="chart_config.categories">
			...
			}" v-for="(serie, index) in props.series" :key="serie.name" @click="handleLegendSelection(index)">
				<div :style="{
		...
		<Teleport to="body">
			...
			<div v-if="aHovered !== -1" class="polarareachart-tooltip chart-tooltip" :style="tooltipPosition">
				<h6 :style="{ color: '#888787' }">
